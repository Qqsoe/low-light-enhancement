...
parser.add_argument('--img_path', type=str,
                    default="./train/INPUT_IMAGES")
parser.add_argument('--img_val_path', type=str,
                    default="./validation/INPUT_IMAGES")
...
# 打印模型结构到日志文件
with open(os.path.join(config.snapshots_folder, 'model_structure.txt'), 'w') as f:
    f.write(str(model))
...
for epoch in range(config.num_epochs):
    ...
    if rank == 0:
        ...
        if SSIM_mean > ssim_high:
            ...
            torch.save(model.state_dict(), os.path.join(config.snapshots_folder, "best_Epoch" + '.pth1'))
